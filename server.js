...
const userRoutes = require('./router/utilisateur');
...
app.get("/", (req, res) => {
  try {
    ...
    console.error(
      "Erreur lors de la récupération des données météo de Mars :",
      error.message
    );
    res.status(500).json({
      error: "Erreur lors de la récupération des données météo de Mars.",
    });
  }
});
